...
test_that("na_approx() | error test", {
  ...
  expect_error(
    na_approx(x = "", index = index, fill_na_tips = TRUE),
    "Assertion on 'x' failed"
  )
  expect_error(
    na_approx(
      x = as.numeric(NA), index = index,
      fill_na_tips = TRUE
    ),
    "Assertion on 'x' failed"
  )
  expect_error(
    na_approx(
      x = c(as.numeric(NA), NA, NA), index = index,
      fill_na_tips = TRUE
    ),
    "Assertion on 'x' failed"
  )
  ...
  na_approx(x = 1:2, index = index, fill_na_tips = TRUE) |>
    expect_error()

  # checkmate::assert_flag(fill_na_tips)
  expect_error(
    na_approx(x = x, index = index, fill_na_tips = 1),
...
test_that("na_locf() | error test", {
  ...
  expect_error(
    na_locf(x = as.numeric(NA), fill_na_tips = TRUE),
    "Assertion on 'x' failed"
  )
  expect_error(
    na_locf(x = c(as.numeric(NA), NA, NA), fill_na_tips = TRUE),
    "Assertion on 'x' failed"
  )

  # checkmate::assert_flag(fill_na_tips)
  expect_error(
    na_locf(x = x, fill_na_tips = 1),
    "Assertion on 'fill_na_tips' failed"
  )
})
...
test_that("na_overall_mean() | general test", {
  ...
  expect_equal(
    na_overall_mean(x),
    c(
      5 + (1 / 3), 1, 5, 10, 5 + (1 / 3), 5, 10, 1, 5 + (1 / 3), 10, 1,
      5, 5 + (1 / 3), 5 + (1 / 3)
    )
  )
})
...
test_that("na_overall_mean() | error test", {
  ...
  expect_error(
    na_overall_mean(x = as.numeric(NA)),
    "Assertion on 'x' failed"
  )
  expect_error(
    na_overall_mean(x = c(as.numeric(NA), NA, NA)),
    "Assertion on 'x' failed"
  )
})
...
test_that("na_overall_median() | error test", {
  ...
  expect_error(
    na_overall_median(x = as.numeric(NA)),
    "Assertion on 'x' failed"
  )
  expect_error(
    na_overall_median(x = c(as.numeric(NA), NA, NA)),
    "Assertion on 'x' failed"
  )
})
...
test_that("na_overall_mode() | general test", {
  ...
  na_overall_mode(x) |>
    expect_equal(c(5, 1, 5, 10, 5, 5, 10, 1, 5, 10, 1, 5, 5, 5, 5))
})
...
test_that("na_spline() | general test", {
  ...
  expect_equal(
    na_spline(x, index),
    c(
      4.567728, 1, 5, 10, 6.589146, 5, 10, 1, 5.037198, 10, 1,
...
test_that("na_spline() | error test", {
  ...
  expect_error(
    na_spline(x = as.numeric(NA), index = index),
    "Assertion on 'x' failed"
  )
  expect_error(
    na_spline(x = c(as.numeric(NA), NA, NA), index = index),
    "Assertion on 'x' failed"
  )

  # prettycheck::assert_identical(x, index, type = "length")
  na_spline(x = 1:2, index = index) |> expect_error()
})
...
test_that("na_weekly_mean() | general test", {
  ...
  expect_equal(
    ...
    c(
      5 + (1 / 3), 1, 5, 10, 5 + (1 / 3), 5, 10, 1, 5 + (1 / 3), 10, 1, 5,
      5, 5
    )
  )
...
test_that("na_weekly_mean() | error test", {
  ...
  expect_error(
    na_weekly_mean(
      x = "", index = index, fill_na_tips = TRUE,
      week_start = 1
    ),
    "Assertion on 'x' failed"
  )
  ...
  # prettycheck::assert_identical(x, index, type = "length")
  na_weekly_mean(x = 1:2, index = index) |>
    expect_error()

  # checkmate::assert_flag(fill_na_tips)
  expect_error(
    na_weekly_mean(
      x = x, index = index, fill_na_tips = 1,
      week_start = 1
    ),
    "Assertion on 'fill_na_tips' failed"
  )
  ...
  expect_error(
    na_weekly_mean(
      x = x, index = index, fill_na_tips = TRUE,
      week_start = 2
    ),
...
test_that("na_zero() | error test", {
  ...
  expect_error(
    na_zero(x = c(as.numeric(NA), NA, NA)),
    "Assertion on 'x' failed"
  )
})
...
test_that("na_plot() | error test", {
  ...
  # checkmate::assert_numeric(x, min.len = 1, all.missing = FALSE)
  expect_error(
    na_plot(x = "", index = index, intp = intp, print = TRUE),
    "Assertion on 'x' failed"
  )
  expect_error(
    na_plot(
      x = as.numeric(NA), index = index, intp = intp,
      print = TRUE
    ),
    "Assertion on 'x' failed"
  )
  ...
  #                           null.ok = TRUE)
  expect_error(
    na_plot(x = x, index = index, intp = "", print = TRUE),
    "Assertion on 'intp' failed"
  )
  expect_error(
    na_plot(
      x = x, index = index, intp = as.numeric(NA),
      print = TRUE
    ),
    "Assertion on 'intp' failed"
  )
  expect_error(
    na_plot(
      x = x, index = index, intp = c(as.numeric(NA), NA, NA),
      print = TRUE
    ),
    "Assertion on 'intp' failed"
  )
  ...
  # prettycheck::assert_identical(x, index, type = "length")
  na_plot(x = 1:2, index = index, intp = NULL, print = TRUE) |>
    expect_error()

  # if (!is.null(intp)) prettycheck::assert_identical( ...
  na_plot(x = x, index = index, intp = 1:2, print = TRUE) |>
    expect_error()

  # checkmate::assert_flag(print)
  expect_error(
    na_plot(x = x, index = index, intp = NULL, print = 1),
...
test_that("na_tip_correction() | general test", {
  ...
  expect_equal(
    na_tip_correction(x = x, intp = intp_1, fill_na_tips = TRUE),
  ...
  expect_equal(
    na_tip_correction(x = x, intp = intp_2, fill_na_tips = FALSE),
    c(NA, 1:12, NA)
  )
})
...
test_that("na_tip_correction() | error test", {
  ...
  # checkmate::assert_numeric(x, min.len = 1, all.missing = FALSE)
  expect_error(
    na_tip_correction(x = "", intp = intp, fill_na_tips = TRUE),
    "Assertion on 'x' failed"
  )
  expect_error(
    na_tip_correction(
      x = as.numeric(NA), intp = intp,
      fill_na_tips = TRUE
    ),
    "Assertion on 'x' failed"
  )
  expect_error(
    na_tip_correction(
      x = c(as.numeric(NA), NA, NA), intp = intp,
      fill_na_tips = TRUE
    ),
    "Assertion on 'x' failed"
  )
  ...
  #                           null.ok = TRUE)
  expect_error(
    na_tip_correction(x = x, intp = "", fill_na_tips = TRUE),
    "Assertion on 'intp' failed"
  )
  expect_error(
    na_tip_correction(
      x = x, intp = as.numeric(NA),
      fill_na_tips = TRUE
    ),
    "Assertion on 'intp' failed"
  )
  expect_error(
    na_tip_correction(
      x = x, intp = c(as.numeric(NA), NA, NA),
      fill_na_tips = TRUE
    ),
    "Assertion on 'intp' failed"
  )
  ...
  # checkmate::assert_flag(fill_na_tips)
  expect_error(
    na_tip_correction(x = x, intp = intp, fill_na_tips = 1),
    "Assertion on 'fill_na_tips' failed"
  )
})
