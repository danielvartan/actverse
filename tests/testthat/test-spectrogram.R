...
test_that("compute_interval_periodogram() | general test", {
  ...
  expect_equal(object$int, lubridate::interval(
    "2020-01-01 00:00:00",
    "2020-01-03 00:00:00"
  ))
...
test_that("compute_interval_periodogram() | error test", {
  ...
  # prettycheck::assert_interval(int_i, any.missing = FALSE)
  expect_error(
    compute_interval_periodogram(
      data = data_1, col = "x",
      int_i = 1,
...
test_that("plot_spectrogram() | general test", {
  ...
  per <- periodogram(
    data = data, col = "x", p_min = 1, p_max = 100,
    print = FALSE
  ) |>
    shush()
