 ...
 test_that("npcra_l5_whole_period()", {
         #Random data
         set.seed(1)
         first_date <- as.POSIXct('2015-01-01')
         last_date <- as.POSIXct('2015-01-15')
         ...
         timestamp <- sort(shuffled_timestamp)
         x <- runif(10000, 0, 10000)
         l5_data <- npcra_l5_whole_period(x, timestamp)
         expect_equal(round(l5_data$l5), 4237)
         expect_equal(l5_data$start_date, as.POSIXct("2015-01-10 21:04:00"))
         ...
         #x <- runif(10**6, 0, 10000)
         #l5_data <- npcra_l5_whole_period(x, timestamp)
         #expect_equal(length(l5_data), 2)
         #expect_is(l5_data$l5, 'numeric')
         #expect_is(l5_data$start_date, 'POSIXct')

         #Data that does not complete at least one 5-hour period
         last_date <- as.POSIXct('2015-01-01 04:59:59')
         shuffled_timestamp <- sample(seq(first_date, last_date, by = "min"), 100)
         timestamp <- sort(shuffled_timestamp)
         x <- runif(100, 0, 10000)
         expect_error(npcra_l5_whole_period(x, timestamp))

         #Two equal l5 values for different dates (should return the first)
         last_date <- as.POSIXct('2015-01-02 12:00:00')
         timestamp <- seq(first_date, last_date, by = "hour")
         x <- replicate(length(timestamp), 1000)
         x[1:11] <-replicate(11, 900)
         l5_data <- npcra_l5_whole_period(x, timestamp)
         expect_equal(l5_data$l5, 900)
         expect_equal(l5_data$start_date, timestamp[1])
