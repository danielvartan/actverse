...
#' that comprises each observation until the end of a 5-hour window, moving on
...
#' Although the first approaches in actigraphy used the L5 for the entire
...
#' npcra_l5(test_log$pim, test_log$timestamp) #expects a tibble 1X2
#'
...
npcra_l5 <- function(x, timestamp, method=1) {
    ...
    l5 <- data.frame()
    if (method==1) {
        out <- npcra_l5_whole_period(x, timestamp)
    }
    if (method==2) {
        out <- npcra_l5_mean_profile(x, timestamp)
    }
    if (method==3) {
        out <- npcra_l5_each_day(x, timestamp)
    }
...
#'rhythm and few arousals (Goncalves et al, 2015), however, it is important
#'to analyze the result for each day (\code{npcra_l5_each_day()})
#'and for the average 24-hour profile (\code{npcra_l5_average_day()})
#'
...
#'
#' Finally, the lowest average activity in 5 hours will be captured and, using
#' the index of the vector of averages, the date of the beginning of the period
#' of 5 hours with the lowest activity will be retrieved.
#'
#'@note
...
#' npcra_l5_whole_period(test_log$pim,
#'                          test_log$timestamp)
#'
...
npcra_l5_whole_period <- function(x, timestamp) {
    ...
    end_window <- timestamp + lubridate::hours(5)
    ...
    for (index in seq_len(index_last_valid_register - 1)) {
        ...
        if (sum_in_5_hours / (window_index-index) < l5) {
            l5 <- sum_in_5_hours / (window_index-index)
            start_date <- timestamp[index]
        }
   }

    out <- dplyr::tibble(l5, start_date)
    out
}

#' Non-Parametric Function L5 (Least Active 5 Hours) for the Mean Profile
#'
#' @description
#'
#' `r lifecycle::badge("experimental")`
#'
#' Calculates the least active window of 5 hours following the mean profile,
...
#' @param timestamp POSIXct/POSIXlt vector that contains the date and time of
#' each observation.
#' @return A tibble of two columns, with the value of L5 in the first column
#' and the start time of the 5 least active period for the mean profile in
#' the second position (to maintain the nomenclature standard,
#' this column is called start_date).
#'
#'@details
#'The L5 is the lowest average activity over a 5-hour period.
#' The first uses of the L5 consisted of calculating the least active 5-hour
#'  period based on the hourly means (WITTING et al., 1990). As it does not
#'  have an explicit definition, it is expected that the values would be
#'  calculated under the complete period
#'  (see \code{npcra_l5_whole_period()}) received and that is why that is
#'  why many authors perform the analysis by two other approaches
#'  (GONCALVES et al., 2015): calculation for the 24h mean profile and for
#'  each day(see \code{npcra_l5_each_day()})
#'
...
#'indexes of the vectors.
#'
#'L5 for the average profile starts by establishing the same date for all
...
#'optimal algorithm).
#'
#'Then begins the calculation of the averages of the 5-hour periods
#'and the search for the smallest one. It is important to note that records
#'whose start time is later than 7 pm include in their 5 hour period in
#'addition to the later hours, include records prior to 7 pm while within
#'the 5 hour window. For example, a record that starts at 20:00:00 will
#'end the 5 hour period at 01:00:00.
#'
#'At the end of the calculation, the lowest average and its respective time
#'is captured
#'
#' @references
#' WITTING, W. et al. Alterations in the circadian rest-activity rhythm in aging
...
#' @examples
#' #Using the test_log data from the package
#' npcra_l5_mean_profile(test_log$pim, test_log$timestamp)
#'
...
#' timestamp <- sort(shuffled_timestamp)
#' x <- runif(1000, 0, 10000)
#' npcra_l5_mean_profile(x, timestamp)
#'
#' @export
npcra_l5_mean_profile <- function(x, timestamp) {
    checkmate::assert_numeric(x)
    checkmate::assert_posixct(timestamp)
    ...
    first_19h_index <- last_valid_index

    if(lubridate::hour(timestamp[last_valid_index]) >= 19) {
        first_19h_index <- which.max(lubridate::hour(data$timestamp) >= 19)
    }

    while (index <= last_valid_index) {
        window_sum <- 0

        if (index < first_19h_index) {
            while (abs(difftime(data$timestamp[window_index],
                                data$timestamp[index], units = "hour")) < 5) {
        ...
        if (sum_in_5_hours / (size_window-index) < l5) {
            l5 <- sum_in_5_hours / (size_window-index)
...
#' Non-Parametric Function M10 (Least Active 5 Hours) for each day
#' @family NPCRA functions
#'
#' @description
#'
...
#' @param timestamp POSIXct/POSIXlt vector that contains the date and time of
#' each observation.
#' @return A tibble of two columns, with the values of L5 in the first column
...
#'
#' @export
npcra_l5_each_day <- function(x, timestamp){
    checkmate::assert_numeric(x)
    checkmate::assert_posixct(timestamp)
    ...
    for (index_day in seq(n_dates)){
        ...
        while (index < index_last_valid_register &
            ...
            if (sum_in_5_hours / (window_index-index) < l5[index_day]) {
                l5[index_day] <- sum_in_5_hours / (window_index-index)
                l5_index[index_day] <- index
            }
