...
#' \eqn{\chi^{2}}{chi square} periodograms to visualize changes in periodicities
#' across intervals within a [`tsibble`][tsibble::tsibble()] object.
#'
#' This function enables the detection of temporal variations in rhythmicity by
#' generating periodograms for consecutive or overlapping time windows.
#'
#' See the [periodogram()][periodogram()] function for details on periodogram
#' computation.
#'
...
spectrogram <- function(
  ...
  if (isTRUE(print)) out$spectrogram |> print()

  invisible(out)
}

find_spectrogram_intervals <- function(
  data,
  int_unit = "days",
  int_n = 7,
  int_step = 720
...
compute_interval_periodogram <- function(
  data,
  col,
  int_i,
  p_unit,
  ...
  prettycheck::assert_interval(int_i, any_missing = FALSE)
  ...
  data <-
    ...
    tsibble::filter_index(
      as.character(lubridate::int_start(int_i)) ~
