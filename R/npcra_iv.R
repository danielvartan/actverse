...
#'
#' Calculates the Intradaily Variability, which identifies the fragmentation of
#' the rest-activity rhythms.
#'
...
#' @details
#'
#' Intradaily variability is a number that is usually between 0 and 2 calculated
#' by dividing the square mean of the first derivative of the data by the
#' population variance (Witting et al., 1990).
#'
...
#' a high variability.
#'
#' If \eqn{X} represents the activity or other numeric variable passed as
#' parameter `x`, the IV follows the formula below:
#'
...
#' @param minutes_interval An integer value representing the duration in minutes
#'   of the time interval for grouping the data. By default, 60 minutes are
#'   considered, this means that the activity will be averaged at hourly
#'   intervals. The minimum value is 1 minute, where all points will be
#'   considered (so there will be no average in intervals).
#'
#' @return A [`numeric`][base::numeric()] value.
#'
...
#' shuffled_timestamp <- sample(seq(first_date, last_date, by = "sec"), 100)
...
#'
#' npcra_iv(act, "x", minutes_interval = 120)
npcra_iv <- function(data, col, minutes_interval = 60) {
    checkmate::assert_numeric(data[[col]])
    checkmate::assert_posixct(data[[tsibble::index(data)]])
    checkmate::assert_int(minutes_interval)

    x <- data[[col]]
    timestamp <- data[[tsibble::index(data)]]

    if (minutes_interval < 1 | minutes_interval > 1440) {
        cli::cli_abort(paste0(
            "{.strong {cli::col_red('minutes_interval')}} must have ",
            "a value between {.strong 1} and {.strong 1440} minutes, ",
            "but is {.strong {minutes_interval}}."
        ))
    }
...
#' @details
#'
#' Intradaily variability (see [npcra_iv()]) is a number that is usually
#' between 0 and 2 calculated by dividing the square mean of the first
#' derivative of the data by the population variance (Witting et al., 1990).
#'
#' Although real records typically have an IV value less than 2, it is possible
#' that this number exceeds 2. IVs greater than 2 are quite common in
#' simulations of random values, since there is no way to define a standard for
#' the data and, therefore, will have a high variability.
#'
...
#' averaging IVs at different time intervals (Goncalves et al., 2014). The
#' function of this package considers a minute limit to calculate the average of
#' IVs.
#'
#' As an example, the default is 60 minutes, so IVs will be calculated with time
#' intervals from 1 to 60 minutes and then average all these values.
#'
#' @param data A [`tsibble`][tsibble::tsibble()] object.
#' @param col A string indicating which column of `data` to use.
#' @param minute_limit An integer value that corresponds to the last minute
#'   interval to group the data. The default is 60, so 60 values of IV will be
#'   calculated to take the average, with the first every minute and the last
#'   every 60 minutes.
#' @param show_messages if set to true it will return the IV for every minute on
#'   the console as a message.
#' @param summarize if set to true, only the IVm value will be output from the
#'   function, if set to false it will also return the IV values for each minute
#'   in a tibble
#'
#' @return The numeric value of IVm if summarize is set as TRUE, otherwise will
#'   be returned a tibble with the IVs until the minute limit with their names
#'   (IVm, IV1, IV2, ...) in the first column and the values of IV in the second
#'   column. The first column is called iv_minute and the second as iv, with the
#'   IVm in the first row.
#'
#' @template references_f
#' @family NPCRA functions
#' @export
...
#'
#' npcra_ivm(act, col = "x", minute_limit = 120, summarize = FALSE)
npcra_ivm <- function(data, col, minute_limit = 60, show_messages = TRUE,
                      summarize = TRUE) {
    checkmate::assert_numeric(data[[col]])
    checkmate::assert_posixct(data[[tsibble::index(data)]])
    ...
    if (minute_limit < 1 | minute_limit > 1440) {
        cli::cli_abort(paste0(
            "{.strong {cli::col_red('minute_limit')}} must have ",
            "a value between {.strong 1} and {.strong 1440} minutes, ",
            "but is {.strong {minute_limit}}."
        ))
    }
    ...
    for (current_minute in seq_len(minute_limit)) {
        current_iv <- npcra_iv(data, col, current_minute)
