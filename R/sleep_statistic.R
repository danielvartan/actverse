#' @title Algorithm that calculates the main variables derived from the activity
#' log, based on the article 'Actigraphy-Based Assessment of Sleep Parameters'
#' by author Desta Fekedulegn
#'
#' @name Statistic
#'
#' @description Algorithm that calculates:
#'              Time in Bed (TIB),
#'              Sleep Period (O-O Interval),
#'              Sleep Minutes during TIB (SMIN),
#'              True Sleep Minutes (TSMIN),
#'              Percent Sleep (PSLP),
#'              Sleep Efficiency (SE),
#'              Mean Activity during TIB (AMEAN),
#'              Median Activity during TIB (AMED),
#'              Activity Standard Deviation during TIB (ASD),
#'              Wake up After Sleep Onset (WASO),
#'              Latency to Persistent Sleep (LPS) and
#'              Wake Minutes during TIB (WMIN).
#'
#'          If you want to see more details about this paper,
#'          we recommend you to access this link:
#'          https://academic.oup.com/annweh/article/64/4/350/5735350?login=false
#'
#'
#' @param analyze
#' A matrix or array containing a vector with dates ,
#' vector with times in the format 'hh:mm:ss', vector with states in the range
#' of (1,2) being:
#'          1 - Sleep
#'          2 - Awake,
#' a vector with ZCM values, vector with PIM values and vector with TAT values
#'
#' @param window_time
#' This parameter sets the minimum continuous value to set sleep. This will be
#' use for calculate Sleep Onset and Sleep Offset (O-O Interval)
#'
#' @param pkg
#' This parameter defines if the algorithm will calculate Mean and
#' Standard Deviation of the Circular time Series
#'
#' This algorithm calculates the variables "mean" and
#' "Standard Deviation" using the Circular Package.
#'
#' Note: If you don't need it,use as a value for the 'pkg' as 'N'.
#'
#'
#' @details
...
#'
#' There are two markers of this line in the algorithm, named "start" and "finish".
#' These markers help to identify the beginning and the end of the sleep period
#' respectively.
#'
...
#' few minutes (defined by the parameter) are marked as a sleep state.
#' If so, it marks the sleep start time, otherwise it advances to the next minute
#' and checks again.
#'
...
#'
#'
#' @return A Dataframe containing the calculated variables:
#'
#'              Week Day,
#'              Date,
#'              Time in Bed (TIB),
...
sleep_statistic <- function(analyze,window_time,pkg) {
